...
-  Apache Flink 存在 Zip Slip 问题：官方已修复，由于某些原因没有分配 CVE
-  Apache Skywalking 潜在 SQL 注入：官方已修复，无法直接触发，不分配 CVE
-  Apache Linkis 存在 Zip Slip 问题：官方已修复，由于某些原因没有分配 CVE
-  Apache OpenWhisk 存在 Zip Slip 问题：官方已修复，由于某些原因没有分配 CVE
...
- https://github.com/vulhub/vulhub 项目 TOP 10 贡献者
- https://github.com/vulhub/java-chains 项目核心贡献者
- jar-analyzer - 可能是最强的开源 Jar 漏洞分析工具
- class-obf - 可能是最强的开源 class 文件混淆工具
- mysql-fake-server - 可能是最受欢迎的 mysql fake server 工具
- shell-analyzer - 第一个开源 GUI 内存马查杀工具
- code-encryptor - 使用 JNI 加密字节码，通过 JVMTI 解密字节码保护代码工具
- poc-runner - 手搓语法解析的最小 xray yaml 规则执行器
- java-gate - 使用 Java JNI 实现各种各样 shellcode 方式
- y4-lang - 不使用任何第三方库从 0 实现的中文脚本语言
- SpringInspector - 字节码污点分析工具作者（已删库）
- JSPKiller - 第一个开源污点分析恶意 JSP 检测工具（已删库）
- FindShell - 第一个开源 Agent 内存马检测和修复工具（已删库）
- Gososerial - 第一个开源跨语言生成 Java 序列化数据工具（已删库）
- accelerator - 第一个开源 Golang 分析 Java 字节码漏洞工具（已删库）
- y4-jdk - 改造 OpenJDK 实现更安全难以利用漏洞的 JDK 8（已删库）
